...
import {
  ...
  Calendar, Plus, RefreshCw, FileText, ShieldCheck,
} from "lucide-react";
...
function RegisterPatientForm({ clinicId, onSubmit, onClose }: {
  clinicId: string;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await onSubmit({ ...form, primary_clinic_id: clinicId });
      onClose();
    } catch (err: any) {
...
function PatientDetailModal({ patient }: { patient: AdminPatient }) {
...
export default function ClinicAdminPatientsPage() {
  const { user } = useAuth();
  const { patients, isLoading, error, fetch, registerPatient, updatePatient, deletePatient } = useAdminPatients();
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={!!detailPatient} onClose={() => setDetailPatient(null)} title="Patient Details" className="max-w-3xl">
        {detailPatient && <PatientDetailModal patient={detailPatient} />}
      </Modal>

      <Modal isOpen={!!editTarget} onClose={() => setEditTarget(null)} title="Edit Patient">
        {editTarget && <EditPatientForm patient={editTarget} onSubmit={(data) => updatePatient(editTarget.id, data)} onClose={() => setEditTarget(null)} />}
      </Modal>
