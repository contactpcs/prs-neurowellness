...
function StaffForm({ initial, clinicId, onSubmit, onClose, isEdit }: StaffFormProps) {
...
export default function ClinicAdminStaffPage() {
  const { user } = useAuth();
  const { staff, isLoading, error, fetch, updateStaff, toggleStaff } = useAdminStaff();
  ...
  return (
    ...
    >
      ...
      <Modal isOpen={!!editMember} onClose={() => setEditMember(null)} title="Edit Staff Member">
        {editMember && user?.clinic_id && (
          <StaffForm initial={editMember} clinicId={user.clinic_id} isEdit onSubmit={(data) => updateStaff(editMember.id, data)} onClose={() => setEditMember(null)} />
        )}
      </Modal>
